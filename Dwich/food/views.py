...
def dwichs(request):
    ...
    ctx = {'dwichs': dwichs}
    return render(request, './food/dwichs.html', ctx)

def salades(request):
    request.session.set_expiry(0)
    salades = Salade.objects.all()
    ctx = {'salades': salades}
    return render(request, './food/salades.html', ctx)

def desserts(request):
    request.session.set_expiry(0)
    desserts = Dessert.objects.all()
    ctx = {'desserts': desserts}
    return render(request, './food/desserts.html', ctx)

def frites(request):
    request.session.set_expiry(0)
    frites = Frite.objects.all()
    ctx = {'frites': frites}
    return render(request, './food/frites.html', ctx)
